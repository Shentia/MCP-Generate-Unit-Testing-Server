...
import { generateUnitTest, generateEnhancedTest } from './tools/testGenerator.js';
...
import { TestGenerationRequest } from './types.js';
  ...
  try {
    switch (name) {
      ...
      case 'install_test_framework': {
        const { projectPath, framework, autoInstall } = args as {
          projectPath: string;
      ...
      case 'generate_unit_test': {
        const request = args as unknown as TestGenerationRequest;
      ...
      case 'validate_test': {
        const { testFilePath, framework } = args as {
          testFilePath: string;
          framework: string;
        };
      ...
      case 'display_side_by_side': {
        const { sourceFilePath, testFilePath } = args as {
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
  ...
  if (name === TEST_GENERATION_PROMPTS.generateUnitTest.name) {
    const { sourceCode, framework, coverageTarget } = args as unknown as {
      sourceCode: string;
      framework: string;
