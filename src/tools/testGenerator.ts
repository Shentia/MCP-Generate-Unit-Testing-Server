import { TestGenerationRequest, TestGenerationResult, CodeAnalysisResult } from '../types.js';
...
export async function generateUnitTest(request: TestGenerationRequest): Promise<TestGenerationResult> {
  try {
    // Step 1: Analyze the source code
    const analysis: CodeAnalysisResult = await analyzeCode(request.filePath);
...
export async function generateEnhancedTest(
