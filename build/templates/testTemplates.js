export class JestTemplate {
    generateImports(analysis, testFramework) {
        const imports = [];
    ...
    generateTestSuite(analysis, framework) {
        const moduleName = analysis.filePath.split('/').pop()?.replace(/\.(ts|js)x?$/, '') || 'module';
        let tests = '';
    ...
    generateFunctionTest(func, sourceFile) {
        const testCases = [];
        testCases.push(`
  describe('${func.name}', () => {
...
`);
    ...
    generateClassTest(cls, sourceFile) {
        let test = `
  describe('${cls.name}', () => {
...
export class JasminKarmaTemplate {
    generateImports(analysis, testFramework) {
        const imports = [];
        const moduleName = analysis.filePath.split('/').pop()?.replace(/\.(ts|js)x?$/, '') || 'module';
        const exportedItems = analysis.exports.map(e => e.name);
