...
export class DocumentationFetcher {
    ...
    async fetchJestDocs() {
        ...
        try {
            ...
            const fetchPromises = urls.map(url => fetch(url).then(res => res.text()).catch(() => ''));
    ...
    async fetchJasmineDocs() {
        const now = new Date();
        // Check if cache is valid
        if (this.cache.jasmine &&
...
//# sourceMappingURL=documentationFetcher.js.map